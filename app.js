  ...
  if (!rows.length) {
    ...
    tbody.innerHTML = rows.map(function(pour) {
      ...
      const testCells = ['1 Day','3 Days','7 Days','14 Days','21 Days','28 Days'].map(function(age) {
        ...
        if (!t) return '<td style="text-align:center;color:var(--text-2);font-size:10px">--</td>';
        const isPassed = t.result === 'Passed';
        const color = isPassed ? '#3B6D11' : '#A32D2D';
        const bg    = isPassed ? '#EAF3DE' : '#FCEBEB';
        return '<td style="text-align:center">'+
          '<div style="font-weight:600;font-size:11px">'+t.average+' PSI</div>'+
          '<span style="font-size:9px;padding:1px 6px;border-radius:6px;background:'+bg+';color:'+color+';font-weight:600">'+t.result+'</span>'+
          '</td>';
      }).join('');
