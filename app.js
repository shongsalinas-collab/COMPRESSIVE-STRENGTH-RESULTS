...
function updatePourInDb(pourId, data) {
  if (!isOnline || !db) return Promise.reject(new Error('Offline'));
  return db.ref('pours/'+pourId).update(data);
}
...
function renderLog() {
  ...
  if (!rows.length) {
    tbody.innerHTML='<tr class="empty-row"><td colspan="13">No pours logged yet. Click "+ Log Pour" to start.</td></tr>';
  } else {
...
function saveTestResult() {
  ...
  updatePourInDb(pourId, { ['tests/'+testKey]: testEntry })
    .then(()=>{
      closeTestModal();
